# PowerShell script para desplegar la versión TF-IDF mejorada con eventos de usuario corregidos

Write-Host "Iniciando despliegue de la versión TF-IDF mejorada con eventos de usuario corregidos..." -ForegroundColor Green

# Configuración
$ProjectID = "retail-recommendations-449216"
$Region = "us-central1"
$ServiceName = "retail-recommender-tfidf-fixed"
...
if ($ServiceUrl) {
    ...
    Write-Host "Probando el registro de eventos de usuario..." -ForegroundColor Yellow
    try {
        $Headers = @{
            "X-API-Key" = "2fed9999056fab6dac5654238f0cae1c"
        }
        $TestUserId = "test_user_system"
...
Write-Host "NOTA: Esta versión corrige el problema con el registro de eventos de usuario" -ForegroundColor Yellow
Write-Host "      y mejora la forma en que se registran los eventos en Google Cloud Retail API." -ForegroundColor Yellow
...
Write-Host "      Ejemplos de endpoints para registrar eventos:" -ForegroundColor Yellow
Write-Host "      POST /v1/events/user/{user_id}?event_type=detail-page-view&product_id=123" -ForegroundColor Cyan
Write-Host "      POST /v1/events/user/{user_id}?event_type=add-to-cart&product_id=123" -ForegroundColor Cyan
Write-Host "      POST /v1/events/user/{user_id}?event_type=purchase-complete&product_id=123" -ForegroundColor Cyan
Write-Host ""
