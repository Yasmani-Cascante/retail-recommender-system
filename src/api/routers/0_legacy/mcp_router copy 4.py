        ...
        if state_manager:
            try:
                ...
                if conversation_session:
                    try:
                        # ✅ AGREGAR EL NUEVO TURN - ESTA ERA LA LÍNEA FALTANTE
                        await state_manager.add_conversation_turn(
                            ...
                            intent_info={"intent": "processing", "confidence": 0.5},
                            market_info={"market_id": conversation.market_id}
                        )
                        logger.info(f"✅ TURN ADDED: New turn added for session {conversation_session.session_id}")
                        
                        # ✅ AHORA SÍ - Extraer turn_number CORRECTO
                        real_session_id = conversation_session.session_id
                        turn_number = len(conversation_session.turns)  # Ahora tiene el turn recién agregado
    ...
    try:
        ...
        try:
            ...
            response_dict = await asyncio.wait_for(
                ...
                timeout=7.0  # Timeout más agresivo de 5 segundos
            )
