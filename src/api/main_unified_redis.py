...
# ── OPCIÓN A (29/06/2026): readiness gateado en warmup de LLMs ────────────
# Por qué: startup_complete se pone True al final del cuerpo de lifespan(),
# pero los warmups de LFM (PASO 8.5c) y GPT-4o-mini (PASO 8.5d) se lanzan
# con asyncio.create_task() ANTES de ese punto -- son "fire-and-forget" por
# diseno, para no bloquear el arranque. Eso significa que startup_complete
# se vuelve True sin esperar a que esos warmups terminen.
# Cloud Run hoy usa un startup probe TCP por defecto (no hay --startup-probe
# configurado en el deploy script) -- enruta trafico en cuanto el puerto
# escucha, mucho antes de que el LLM este caliente. Este flag, leido por el
# nuevo endpoint /health/startup-probe, le da a Cloud Run una senal real de
# "listo" que si espera a ambos warmups.
llm_warmup_complete = False  # True cuando AMBOS warmups (LFM + GPT-4o-mini) terminaron
_llm_warmup_pending = 0      # Contador de warmups aun en vuelo; se inicializa a 2 antes de lanzarlos
...
try:
    ...
    # OPM-1: Visual index reconciliation job
    from src.api.services.visual_index_sync_job import run_visual_index_sync_job
...
async def lifespan(app: FastAPI):
    ...
    global _llm_warmup_pending  # OPCION A (29/06/2026): contador de warmups LFM/GPT-4o-mini
    ...
    try:
        ...
        # 09.06.2026 
        # Este bloque pre-warming de Claude Haiku queda comentado porque 
        # hemos reemplazado Claude por GPT-4o-mini para fallback en mcp_personalization_engine.py, 
        # y GPT-4o-mini no tiene el mismo problema de conexión en frío.
        # ════════════════════════════════════════════════════════════════════
        ...
        async def _warmup_lfm_connection() -> None:
            ...
            global llm_warmup_complete, _llm_warmup_pending
            try:
                ...
                # OPCION A: este warmup termino (exito, timeout o error) -- cuenta
                # igual. No queremos que un fallo de OpenRouter bloquee el
                # readiness para siempre; con esto la instancia queda "lista"
                # aunque el warmup haya fallado, en vez de quedar 503 eternamente.
                _llm_warmup_pending -= 1
                if _llm_warmup_pending <= 0:
                    llm_warmup_complete = True
                    logger.info("llm_warmup_complete_all: ambos warmups terminados")

        asyncio.create_task(_warmup_lfm_connection())
        logger.info("LFM warm-up started in background (non-blocking)")

        # ── PASO 8.5d: GPT-4o-mini WARM-UP (background) ────────────────────
        # FIX (29/06/2026 -- DCT seccion 33, pendiente del sprint de
        # coherencia categorica): la generacion de mensaje de
        # /v1/mcp/visual-search (_generate_visual_search_message en
        # visual_search_router.py) usa LFM2-24B primario + GPT-4o-mini de
        # respaldo, con timeouts cortos (4.0s/3.0s). En produccion (sesion
        # 28/06/2026), las dos primeras consultas justo despues de un
        # deploy fallaron con AMBOS modelos agotando su tiempo
        # ("error": "" -- firma de asyncio.TimeoutError), degradando al
        # mensaje estatico de respaldo. Una tercera consulta, minutos
        # despues, funciono en 2s.
        #
        # LFM ya tiene warmup de arranque (PASO 8.5c, arriba) y keep-alive
        # periodico (PASO 8.7, abajo) -- el calentamiento real ocurre en la
        # replica de Together.ai (lado del proveedor), no en la conexion
        # local, asi que ese warmup YA beneficia tambien las llamadas de
        # visual-search al mismo modelo. El hueco real es GPT-4o-mini, que
        # nunca se calentaba al arrancar (el codigo de PASO 8.6 que lo
        # mencionaba esta comentado, ya que originalmente era para Claude).
        #
        # Nota: el propio codigo de este archivo documenta que GPT-4o-mini
        # "no tiene el mismo problema de conexion en frio" que Claude/LFM --
        # este warmup es defensivo (decision explicita de Yasmani de
        # calentar ambos modelos), no una respuesta a un cold-start de
        # replica confirmado especificamente para GPT-4o-mini. Costo
        # marginal (1 llamada de 1 token al arrancar).
        async def _warmup_gpt4o_mini_connection() -> None:
            """Calienta la conexion con OpenRouter GPT-4o-mini (defensivo)."""
            global llm_warmup_complete, _llm_warmup_pending
            try:
                await asyncio.sleep(8.0)  # Esperar a que el startup termine

                if os.environ.get('GPT4O_MINI_FALLBACK_ENABLED', 'false').lower() != 'true':
                    logger.info("gpt4o_mini_warmup_skipped: not enabled")
                    return
                ...
                logger.info("gpt4o_mini_warmup_started")
                resp = await asyncio.wait_for(
                    client.complete(
                        "Eres un asistente util.",
                        "Responde hola.",
                    ),
                    timeout=14.0  # mismo presupuesto generoso que LFM
                )
                logger.info("gpt4o_mini_warmup_complete model=%s tokens_out=%d",
                            resp.model, resp.output_tokens)
            except asyncio.TimeoutError:
                logger.warning("gpt4o_mini_warmup_timeout: > 14s")
            except Exception as _e:
                logger.warning("gpt4o_mini_warmup_error: %s", _e)
            finally:
                # OPCION A: mismo razonamiento que en _warmup_lfm_connection --
                # cuenta como terminado aunque haya fallado, para no bloquear
                # el readiness para siempre si OpenRouter esta caido.
                _llm_warmup_pending -= 1
                if _llm_warmup_pending <= 0:
                    llm_warmup_complete = True
                    logger.info("llm_warmup_complete_all: ambos warmups terminados")

        # OPCION A: inicializar el contador a 2 justo antes de lanzar AMBAS
        # tareas de warmup -- si se inicializara antes (ej. junto a LFM, mas
        # arriba) y esa primera tarea terminara excepcionalmente rapido por
        # un error temprano, podria decrementar antes de que esta segunda
        # tarea exista, dando un falso "completo" prematuro con solo 1 de 2
        # warmups realmente intentados. Se inicializa una sola vez, aqui,
        # cuando ambas funciones ya estan definidas y listas para lanzarse.
        _llm_warmup_pending = 2
        ...
        # (09/06/2026) Este paso ya no es necesario porque hemos reemplazado Claude por GPT-4o-mini para fallback, 
        # y GPT-4o-mini no tiene el mismo problema de conexión en frío ni de keep-alive. 
        # Sin embargo, dejo el código comentado aquí para referencia futura en caso de que volvamos a usar modelos con conexiones TCP similares a Claude.
        # ════════════════════════════════════════════════════════════════════
        ...
        if SHOPIFY_KB_AVAILABLE and settings.KB_USE_SHOPIFY_CMS:
            ...
            try:
                ...
                # DB connection: DATABASE_URL has priority (Supabase / Cloud Run).
                # Falls back to individual DB_* vars for local development.
                # os.environ is read directly (not via settings.*) to guarantee
                # Cloud Run injected vars always take precedence over .env defaults.

                # ── DATABASE_URL tiene prioridad (igual que alembic/env.py) ─────────────────
                # Supabase, Neon y Cloud Run usan una sola URL en vez de 5 variables.
                # Si DATABASE_URL no existe, caemos en las variables individuales (backward-compat).
                ...
                if _database_url_raw:
                    ...
                    logger.info("db_config_from_DATABASE_URL",
                                host=db_host_final, port=db_port_final,
                                database=db_name_final, ssl=db_ssl_final)
                else:
                    # DATABASE_URL is required — individual DB_* variables are no longer supported.
                ...
                try:
                    db_pool = await asyncpg.create_pool(
                        ...
                        command_timeout=60,
                        # ✅ REQUERIDO para Supabase Supavisor (session pooler):
                        # desactiva el caché de prepared statements de asyncpg.
                        # Sin esto, el pooler puede rotar el backend y el statement
                        # cacheado apunta a una conexión que ya no existe → error silencioso.
                        # Impacto de performance: despreciable (KB tiene ~26 páginas).
                ...
                # Reconciliación periódica del índice FAISS con el catálogo TF-IDF.
                # Complementa los webhooks products/create y products/update.
                # Solo arranca si VISUAL_SEARCH_ENABLED=true y hay catálogo en memoria.
                # Ver src/api/services/visual_index_sync_job.py para documentación completa.
                if (
                    os.environ.get("VISUAL_SEARCH_ENABLED", "false").lower() == "true"
                    and tfidf_recommender
                    and getattr(tfidf_recommender, "loaded", False)
                    and tfidf_recommender.product_data
                ):
                    app.state.visual_index_sync_task = asyncio.create_task(
                        run_visual_index_sync_job(tfidf_recommender.product_data)
                    )
                    logger.info(
                        "opm1_visual_sync_job_scheduled",
                        interval_hours=os.getenv("PRODUCT_VISUAL_SYNC_INTERVAL_HOURS", "6"),
                        initial_delay_seconds=os.getenv("PRODUCT_VISUAL_SYNC_INITIAL_DELAY_S", "120"),
                        catalog_products=len(tfidf_recommender.product_data),
                        note="FAISS gap reconciliation active — complements webhooks",
                    )
                else:
                    app.state.visual_index_sync_task = None
                    logger.info(
                        "opm1_visual_sync_job_skipped",
                        visual_search_enabled=os.environ.get("VISUAL_SEARCH_ENABLED", "false"),
                        has_catalog=bool(
                            tfidf_recommender
                            and getattr(tfidf_recommender, "loaded", False)
                            and tfidf_recommender.product_data
                        ),
                        note="enable VISUAL_SEARCH_ENABLED=true to activate OPM-1",
                    )

                # ── PASO 10.6: Pre-calentar colbert_client singleton para outfit search ────────
                # FIX (13/05/2026): El primer request al endpoint /v1/mcp/visual-search/outfit
                # tardó 135s porque _auth_headers() bloqueaba esperando el ID token IAM.
                #
                # Root cause: el singleton del visual_search_router se inicializa LAZY
                # (en el primer request de usuario). Su _cached_token está vacío y
                # _fetch_id_token_sync() tarda ~135s en el primer intento en algunas
                # instancias Cloud Run (detectado 13/05/2026, autoscaling instance).
                #
                # Solución: inicializar el singleton Y pre-fetchear el token IAM AHORA,
                # durante el lifespan startup, donde el delay de 135s no afecta al usuario.
                # Para el primer request real, el token ya está en caché (~0ms).
                #
                # NOTA: _auth_headers() ya tiene asyncio.wait_for(timeout=10s) como
                # segunda línea de defensa (FIX en colbert_client.py).
                if os.environ.get('VISUAL_SEARCH_ENABLED', 'false').lower() == 'true':
                    try:
                        ...
                        # Fire-and-forget: no bloqueamos el startup si el prefetch tarda
                        asyncio.create_task(_prefetch_iam_token())
                        logger.info(
                            'visual_search_colbert_warmup_started',
                            note='IAM token prefetch in background — eliminates 135s first-request delay'
                        )
                    except Exception as _warmup_err:
                        logger.warning(
                            'visual_search_colbert_warmup_error',
                            error=str(_warmup_err)
                        )
    ...
    try:
        ...
        try:
            ...
            # OPM-1: Cancel visual index sync job
            if hasattr(app.state, 'visual_index_sync_task') and app.state.visual_index_sync_task:
                try:
                    app.state.visual_index_sync_task.cancel()
                    await asyncio.gather(app.state.visual_index_sync_task, return_exceptions=True)
                    logger.info("✅ OPM-1 visual index sync job stopped cleanly")
                except Exception as e:
                    logger.warning(f"⚠️ OPM-1 visual index sync job shutdown warning: {e}")
...
# ═════════════════════════════════════════════════════════════════════════
# OPCION A (29/06/2026): Startup probe dedicado para Cloud Run
# ═════════════════════════════════════════════════════════════════════════
# Endpoint EXCLUSIVO para que Cloud Run decida cuando enrutar trafico real
# a esta instancia. Distinto de /health (que el frontend sigue usando sin
# cambios para su propio overlay y polling -- ver ChatWidget.tsx).
#
# Devuelve 503 hasta que el cuerpo principal de lifespan() haya terminado
# (startup_complete) Y ambos warmups de LLM hayan terminado (llm_warmup_
# complete) -- exito o fallo, no se bloquea indefinidamente si OpenRouter
# esta caido (ver finally en PASO 8.5c/8.5d, arriba).
#
# Sin autenticacion: el startup probe de Cloud Run llama directo al puerto
# del contenedor, sin headers de API key (igual que /health, /metrics).
#
# Configuracion de Cloud Run pendiente (NO aplicada automaticamente --
# requiere `gcloud run services update` aparte, revisar con Yasmani):
#   --startup-probe=httpGet.path=/health/startup-probe,httpGet.port=8080,
#     periodSeconds=2,timeoutSeconds=2,failureThreshold=15
@app.get("/health/startup-probe", include_in_schema=False)
async def cloud_run_startup_probe():
    """Readiness real para Cloud Run -- gateado en warmup de LFM + GPT-4o-mini."""
    if startup_complete and llm_warmup_complete:
        return {"ready": True}
