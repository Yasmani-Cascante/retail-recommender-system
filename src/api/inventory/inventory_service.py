...
"""
Inventory Service - Gestión de Disponibilidad de Productos
==========================================================
...
@dataclass
class InventoryInfo:
    """Información completa de inventario para un producto"""
    product_id: str
...
class InventoryService:
    """
    Servicio principal de gestión de inventario que integra múltiples fuentes
    """
    
    def __init__(self, redis_service: Optional[RedisService] = None):
        self.redis_service = redis_service
        self.shopify_client = None
        # ✅ ARCHITECTURAL FIX: No async tasks in __init__
        # Redis service debe estar listo antes de crear el servicio
        ...
        # ✅ Estadísticas enterprise para observabilidad
        self._stats = {
            "cache_hits": 0,
            "cache_misses": 0,
            "shopify_calls": 0,
        ...
        logger.info(f"🏪 InventoryService v2.0 initialized - Redis: {self.redis_service is not None}")
    
    async def ensure_ready(self):
        """
        ✅ Método para asegurar que el servicio esté listo para operar
        Inicializa componentes que requieren async initialization
        """
        try:
            ...
            logger.warning(f"⚠️ Shopify client initialization failed: {e}")

    async def get_stats(self) -> Dict[str, Any]:
        """
        ✅ Obtener estadísticas del servicio para observabilidad
        """
        stats = self._stats.copy()
        stats["redis_health"] = "unknown"
        
        if self.redis_service:
            try:
                health = await self.redis_service.health_check()
                stats["redis_health"] = health.get("status", "unknown")
            except Exception:
                stats["redis_health"] = "error"
    ...
    async def check_product_availability(
        ...
        """
        Verificar disponibilidad de un producto específico en un mercado.
        ...
        Returns:
            InventoryInfo con información completa de disponibilidad
        """
        try:
            ...
            # 3. Construir información de inventario
            ...
            logger.info(f"✅ Inventory check for {product_id}: {inventory_info.status.value} ({inventory_info.available_quantity} units)")
    ...
    async def check_multiple_products_availability(
        ...
        """
        Verificar disponibilidad de múltiples productos en paralelo.
        ...
        try:
            ...
            logger.info(f"✅ Checked inventory for {len(product_ids)} products in market {market_id}")
    ...
    async def enrich_products_with_inventory(
        ...
        """
        Enriquecer lista de productos con información de inventario.
        ...
        Returns:
            Lista de productos con información de inventario añadida
        """
        try:
            ...
            for product in products:
                ...
                if product_id and product_id in inventory_data:
                    ...
                    # Añadir campos de inventario al producto
                    ...
                    # Producto sin información de inventario - usar fallback
                    enriched_product = product.copy()
                    enriched_product.update({
            ...
            logger.info(f"✅ Enriched {len(enriched_products)} products with inventory data")
    ...
    async def _cache_inventory_info(self, inventory_info: InventoryInfo, market_id: str):
        ...
        try:
            ...
            logger.debug(f"Error caching inventory info: {e}")
    ...
    async def _get_shopify_inventory(self, product_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Intentar obtener información de inventario de Shopify
            # Nota: Esta implementación depende de la API específica de Shopify
            product_data = await self._fetch_shopify_product(product_id)
            
            if product_data:
                # Extraer información de inventario de la respuesta de Shopify
                inventory_data = {
    ...
    async def _fetch_shopify_product(self, product_id: str) -> Optional[Dict]:
        ...
        try:
            ...
            # En producción, esto se conectaría a la API real de Shopify
            
            # Simulación de datos de Shopify
            simulated_data = {
                "id": product_id,
            ...
            # Añadir algo de variabilidad para testing
            import random
    ...
    async def _build_inventory_info(
        ...
        if quantity == 0 and shopify_data.get("available", True):
            # Si Shopify dice que está disponible pero no hay cantidad, asumir stock bajo
            quantity = 3
        
        # Construir información de inventario
        inventory_info = InventoryInfo(
            product_id=product_id,
            status=InventoryStatus.AVAILABLE,  # Se calculará en __post_init__
            quantity=quantity,
            reserved_quantity=0,  # Podría obtenerse de Shopify en implementación real
    ...
    async def _create_fallback_inventory(self, product_id: str, market_id: str) -> InventoryInfo:
        """Crear información de inventario de fallback optimista"""
        ...
        if self.redis_service:
            try:
                ...
                await self.redis_service.set_json(cache_key, cache_data, self.FALLBACK_TTL)
            except Exception as e:
                logger.warning(f"Error caching fallback inventory: {e}")
        
        logger.info(f"🔄 Created fallback inventory for {product_id} in market {market_id}")
...
def create_inventory_service(redis_service: Optional[RedisService] = None) -> InventoryService:
    """
    ✅ Factory para crear InventoryService con RedisService enterprise
    
    ⚠️ DEPRECATED: Usar ServiceFactory.create_inventory_service() para nueva arquitectura
    """
    return InventoryService(redis_service=redis_service)
