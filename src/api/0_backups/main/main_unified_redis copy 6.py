...
from src.api.factories.service_factory import ServiceFactory, health_check_services
...
# 🚀 STEP 2: PERFORMANCE OPTIMIZATION - CONSERVATIVE INTEGRATION
from src.api.core.mcp_router_conservative_enhancement import apply_performance_enhancement_to_router
# Aplicar mejoras conservadoras - ACTIVADO
# CORRECCIÓN: Acceder al objeto router dentro del módulo
mcp_router.router = apply_performance_enhancement_to_router(mcp_router.router)
logger.info("✅ STEP 2: Conservative performance enhancements applied to MCP router")
...
# Incluir router de productos - ✅ ENTERPRISE INTEGRATION
...
async def fixed_startup_event():
    ...
    logger.info(f"🔧 EXPLICIT: mcp_state_manager initialization completed. Result: {type(mcp_state_manager) if mcp_state_manager else 'None'}")    # RESUMEN FINAL DEL STARTUP
    # ==========================================
...
async def enterprise_health_check():
    """
    ✅ Health check enterprise comprehensivo usando ServiceFactory
    
    Valida todos los servicios enterprise incluyendo:
    - RedisService enterprise
    - InventoryService con dependency injection
    - ProductCache hybrid
    - Todos los componentes del sistema
    """
    try:
        # ✅ Health check de todos los servicios enterprise
        enterprise_health = await health_check_services()
        
        # ✅ Añadir información del sistema principal
        enterprise_health["main_system"] = {
            "uptime_seconds": time.time() - start_time,
            "redis_client_legacy": redis_client is not None,
            "product_cache_legacy": product_cache is not None,
            "hybrid_recommender": hybrid_recommender is not None,
            "mcp_recommender": mcp_recommender is not None
        }
...
# ============================================================================
# 🔄 LEGACY ENDPOINTS - Mantener durante transición
# ============================================================================

@app.get("/health")
# async def emergency_health_check():
#     """Emergency health check with <2s response guarantee"""
#     try:
