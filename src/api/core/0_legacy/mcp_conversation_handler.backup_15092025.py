    ...
    try:
        ...
        if mcp_engine and mcp_context and base_recommendations:
            try:
                ...
                # ✅ MEJORADO: Incluir información de estado conversacional para diversificación
                cache_context = {
                    "market_id": market_id,
                    "user_segment": "standard",  # Podría mejorarse con segmentación real
                    "product_categories": ["general"],  # Mantener estable para cache hits
                    "conversation_turn": mcp_context.total_turns if mcp_context else 1,  # ✅ NUEVO
                    "is_follow_up_request": mcp_context.total_turns > 1 if mcp_context else False,  # ✅ NUEVO
                    "shown_products_count": len([turn.recommendations_provided for turn in mcp_context.turns if hasattr(turn, 'recommendations_provided') and turn.recommendations_provided]) if mcp_context else 0  # ✅ NUEVO
                }
                    ...
                    final_response.update({
                        ...
                        "metadata": {
                            **final_response["metadata"],
                            ...
                            "diversification_applied": mcp_context.total_turns > 1 and recommendations_to_use == base_recommendations if mcp_context else False  # ✅ NUEVO
                        }
                    })
                    ...
                    final_response.update({
                        ...
                        "metadata": {
                            **final_response["metadata"],
                            ...
                            "cache_hit": False,
                            "diversification_applied": mcp_context.total_turns > 1 if mcp_context else False  # ✅ NUEVO
                        }
                    })
                    
                    # ✅ NUEVO: Actualizar estado conversacional con nuevas recomendaciones
                    if mcp_context:
                        try:
                            # Extraer IDs de las recomendaciones generadas
                            new_recommendation_ids = [rec.get('id') for rec in final_response["recommendations"] if rec.get('id')]
                            
                            # Crear nuevo turn conversacional
                            from src.api.mcp.conversation_state_manager import ConversationTurn
                            ...
                            mcp_context.turns.append(new_turn)
                            mcp_context.total_turns += 1
                            mcp_context.last_updated = time.time()
                            
                            logger.info(f"✅ Conversation state updated: turn {mcp_context.total_turns}, recommendations: {len(new_recommendation_ids)}")
                            
                        except Exception as state_update_e:
                            logger.warning(f"⚠️ Failed to update conversation state: {state_update_e}")
