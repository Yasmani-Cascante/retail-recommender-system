...
class MCPPersonalizationEngine:
    ...
    async def _determine_optimal_strategy(
        ...
        try:
            ...
            if list(scores.values()).count(max_score) > 1:
                ...
                if len(tied_strategies) > 1:
                    ...
                    await self._track_strategy_effectiveness(
                        selected_strategy, user_id, user_message, max_score, scores
                    )
            ...
            logger.info(
                f"Strategy determination for '{user_message[:30]}...': "
            ...
            await self._track_strategy_effectiveness(
                winning_strategy, user_id, user_message, max_score, scores
            )
    ...
    async def _get_user_strategy_history(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            await self.redis.setex(cache_key, 24 * 3600, json.dumps(basic_profile))
    ...
    async def _track_strategy_effectiveness(
        ...
        try:
            ...
            selection_id = hashlib.md5(
                f"{user_id}_{strategy.value}_{int(time.time())}_{query[:20]}".encode()
            ).hexdigest()[:12]
