...
class MCPConversationStateManager:
    ...
    async def add_conversation_turn_with_recommendations(
        ...
        """
        ✅ MÉTODO ESPECIALIZADO: Crear ConversationTurn con recommendation IDs garantizados
        
        Este método asegura que recommendations_provided se pueble correctamente
        para resolver el problema de diversificación.
        
        Args:
            session: Contexto de conversación existente
            user_query: Query del usuario
            ai_response: Respuesta de AI generada
            recommendation_ids: IDs de productos recomendados (CRÍTICO para diversificación)
        ...
        # ✅ VALIDACIÓN: Asegurar que recommendation_ids sea lista válida
        if not isinstance(recommendation_ids, list):
            recommendation_ids = []
            logger.warning("⚠️ recommendation_ids no es lista, usando lista vacía")
        ...
        new_turn = ConversationTurn(
            ...
            intent_entities=[],
            ai_response=ai_response,
            recommendations_provided=valid_ids,  # 🎯 CRÍTICO: IDs reales para diversificación
            market_context={
    ...
    # ✅ MÉTODO DE COMPATIBILIDAD: Wrapper para el método anterior
    async def add_conversation_turn_simple(
        ...
        Método de compatibilidad - usa el nuevo método especializado sin recommendation_ids
        """
        logger.info(f"✅ CONVERSATION CONTEXT FROM  add_conversation_turn_simple")
        logger.warning("⚠️ Using compatibility method without recommendations_provided ( without recommendation_ids) - diversification may not work")
        try:
