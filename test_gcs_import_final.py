#!/usr/bin/env python3
"""
Script para probar la importación de productos vía GCS (versión final).
"""
import os
import json
import asyncio
...
async def test_gcs_import():
    ...
    try:
        ...
        logging.info(f"Muestra de datos (primeros 2 productos):")
        for i, product in enumerate(SAMPLE_PRODUCTS[:2]):
            ...
            if i == 0:
                logging.info(f"- Campos: {list(product.keys())}")
        ...
        logging.info(f"¡Prueba de importación completada!")
